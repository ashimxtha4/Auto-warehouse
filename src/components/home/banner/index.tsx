      ...
      <div className='flex justify-center md:my-4'>
        {BANNER_ITEMS.map(item => (
          <Link
            key={item.id}
            href={item.href}
            className='border-primary-main bg-primary-main hover:text-primary-main mx-1 flex w-max items-center gap-2 rounded-full px-2 py-1 text-xs font-medium text-white transition-all hover:bg-white sm:mx-2 sm:px-4 sm:py-2 md:text-2xl'
          >
            {item.label}
            <AiOutlineArrowRight />
          </Link>
